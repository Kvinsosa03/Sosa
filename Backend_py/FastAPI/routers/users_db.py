...
def search_user(field: str, key):
    try:
        user = db_clint.local.users.find_one({field: key})
...
async def user(user: User):
    ...
    id = db_clint.local.users.insert_one(user_dict).inserted_id
    
    new_user = user_schemas(db_clint.local.users.find_one({"_id":id})) 
...
async def users():
    return users_schemas(db_clint.local.users.find())
...
async def user(id: str):
    ...
    found = db_clint.local.users.find_one_and_delete({"_id": ObjectId(id)})
...
async def user(user: User):
    ...
    try:
        ...
        db_clint.local.users.find_one_and_replace({"_id": ObjectId(user.id)}, user_dict)
